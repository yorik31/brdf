...
class processing:
            ...
            def import_band(self,list) :
                ...
                for k,dict in enumerate(list):
                    ...
                    new_file = os.path.join(WD_RES, 'band_' + str(band) + '.tif')
                    #Check if file already import and geocoded
                    geo_file = os.path.join(WD_RES, 'band_' + str(band) + '_geo.tif')
                    ...
                    if (self.gcp_grid_tp_set == False) :
                        ...
                        for k,rec in enumerate(collection_list) :
                            ...
                            collection_out.append(self.geocoded_file(
                                                                rec,
                                                                os.path.join(WD_RES,file_name[k]),
                                                                gcp_list))
            ...
            #    output : EPSG 4326 File
                if not os.path.exists(output_image):
                    (fd, out_name) = tempfile.mkstemp(prefix='with_geometry_',dir = WD,suffix='.tif')
                    os.close(fd)
            ...
            #read image
                    ...
                    if not os.path.exists(output_image):
                        (fd, out_name2) = tempfile.mkstemp(prefix='with_geometry_', dir=WD, suffix='.tif')
                        os.close(fd)
