...
class InputCollection(object):
    ...
    def resampleToReference(self, ds, nullValList, workingRegion, resamplemethod, tempdir='.', useVRT=False):
        ...
        import tempfile
        # the src prf file
        (fileh,src_prf) = tempfile.mkstemp('.prf',dir=tempdir) #Error
        ...
        with open(log_file, 'a') as fb:
            fb.write(str(tempdir)+'\n')
            fb.write(str(temp_image)+'\n')
            fb.write(str(infile)+'\n')
            fb.write(str(src_prf)+'\n')
            fb.write(str(dest_prf)+'\n')
    ...
    def resampleAllToReference(self, footprint, resamplemethodlist, tempdir='.', useVRT=False):
        """
        Resamples all datasets that don't match the reference to the
